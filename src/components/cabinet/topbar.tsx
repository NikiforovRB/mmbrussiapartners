  ...
  return (
    <header className="sticky top-0 z-20 px-1 pt-4">
      <div className="rounded-panel bg-white/70 backdrop-blur-xl px-3 sm:px-5 py-3 flex items-center gap-3 sm:gap-4">
        ...
        {onSearch ? (
          <div className="hidden md:flex items-center gap-2 rounded-panel bg-card-light px-3.5 h-10 w-72">
        ...
        {rightSlot}
        <button className="relative grid h-10 w-10 place-items-center rounded-btn bg-card-light text-ink-muted hover:text-ink">
          <Bell className="h-4 w-4" />
        </button>
        <div ref={ref} className="relative">
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex items-center gap-2.5 rounded-btn bg-card-light pl-1.5 pr-3 h-10"
          >
          ...
          <AnimatePresence>
            {open ? (
              <motion.div
                ...
                className="absolute right-0 top-12 w-60 rounded-panel bg-white p-2"
                style={{ boxShadow: "0 24px 60px -24px rgba(11,16,32,0.18)" }}
              >
                <div className="px-3 py-2 text-xs text-ink-muted">{user.email}</div>
                <Link
                  href={resolvedProfileHref}
                  className="flex items-center gap-2.5 rounded-panel px-3 py-2 text-sm hover:bg-card-light"
                >
                  <UserIcon className="h-4 w-4" /> Профиль
                </Link>
                <button
                  onClick={() => signOut({ callbackUrl: "/" })}
                  className="w-full flex items-center gap-2.5 rounded-panel px-3 py-2 text-sm text-danger hover:bg-card-light"
                >
