...
## «Дилерская сеть» на mmbrussia.ru

Представитель включает «Показывать телефон на сайте» в профиле — это заявка. Администратор с правом `dealers.approve` одобряет или отклоняет её в карточке представителя (список заявок — фильтр «Заявки на публикацию» в «Представителях»). На сайте телефон есть, пока выполнены три условия: согласие представителя, одобренная публикация и активная учётная запись.

Кабинет сам отправляет изменения в `POST {MMB_SITE_URL}/api/integrations/dealers` (`src/lib/site-dealers.ts`, только сервер):

- одобрение, смена телефона, города, страны или подписи у опубликованного — `upsert`;
- выключенный тоггл (отзыв согласия, без модерации), отклонение или снятие публикации, блокировка, удаление представителя — `remove`;
- раз в сутки и по кнопке в «Настройки → Дилерская сеть на сайте» — `sync` полным списком (там же проверка без изменений и журнал обменов).

Отправка идёт после ответа администратору, при сбое сети или 5xx повторяется через 1, 5 и 30 минут, остальное добирает суточная сверка. На сайт уходят `externalId` (id пользователя), телефон, город, страна (по умолчанию «Россия») и подпись; адрес и email не отправляются. Переменные — `MMB_SITE_URL` и `MMB_DEALERS_SYNC_SECRET` (см. `.env.example`), без них интеграция выключена.

## Публичный API для mmbrussia.ru

`GET /api/public/representatives` — JSON со списком представителей, чью публикацию телефона одобрил администратор:
